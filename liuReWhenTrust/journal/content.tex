...
\begin{algorithm}
\caption{Model-Based Policy Optimization with Deep Reinforcement Learning}
\begin{algorithmic}[1]
...
\begin{algorithm}
\caption{Re-implemented Model-Based Policy Optimization with Deep Reinforcement Learning}
\begin{algorithmic}[1]
\State Initialize policy $\pi_{\phi}$, predictive model $p_{\theta}$, environment dataset $D_{env}$, model dataset $D_{model}$
\For{T steps}
